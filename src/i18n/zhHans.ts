...
export const zhHans = {
  ...
  settings: {
    ...
    policyRetainNoTrain: '不用于训练，会短期保留用于滥用监控',
    policyMayTrain: '可能用于改进模型，包括人工审阅',
    policyUnknown: '未核实，请自行查阅其隐私政策',
  ...
  diagnostics: {
    ...
    finishReasonMissing: '（接口未返回）',
    truncatedNote: '响应被截断了：模型还没写完就达到了输出长度上限。可以先把分析范围调短（设置 → 分析范围），或换一个输出上限更高的模型。',
    responseLength: (n: number) => `内容长度 ${n} 字符`,
    rawHeader: '模型原始返回',
    rawEmpty: '（模型没有返回任何内容）',
    copy: '复制诊断信息',
    copied: '已复制到剪贴板',
